...
GEF_kinetics <- GEF_kinetics %>% 
  mutate('kcat_Km' = kcat_Km/WT_GEF) %>% 
  select(mutant, kcat_Km)
GAP_kinetics <- GAP_kinetics %>% 
  mutate('kcat_Km' = kcat_Km/WT_GAP) %>% 
  select(mutant, kcat_Km)
kinetics <- GEF_kinetics %>% 
  inner_join(., GAP_kinetics, by = 'mutant') %>% 
  select(mutant, 'GEF' = kcat_Km.x, 'GAP' = kcat_Km.y)
...
plot <- data %>% 
  ggplot(aes(x = log(GEF), y = log(GAP))) +
  geom_point(aes(shape = tag, color = difference2), size = 6) +
...
cairo_pdf(file = 'Figure4_Multispecificity/APMS_and_biophysics_combined/kinetics_vs_AP-MS_semicircle_plot_diff.pdf', family = "Arial Unicode MS", width = 2.2, height = 2.6)
plot
dev.off()
