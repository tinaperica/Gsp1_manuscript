...
### 1) corr_of_corr_with_SGA/filtered_correlations.RData keeps only the queries that have at least 10 siginificant interactions with array genes that have significant interactions with Gsp1 mutants
### 2) corr_of_corr_with_SGA/filtered_v2_correlations.RData adds another filter, it discards the queries from filtered_correlations that don't have a significant positive correlation with any of the Gsp1 mutants
library(tidyverse)
library(dendextend)
library(factoextra)
...
sga_index <- read_tsv('Data/spitzemap_name2ORF_index.txt') %>% 
...
#### do the bonferroni and FDR correction  for the correlation p-values - RUN THIS ONCE
...
# of the correlations with at least one signficant corr, how many
# significant ones do they have?
...
save(filtered_correlations, file = 'Data/filtered_v5_correlations.RData')
...
save(filtered_correlations, file = 'Data/filtered_v6_correlations.RData')
...
### for all the downstream analysis use the version 5 - 
