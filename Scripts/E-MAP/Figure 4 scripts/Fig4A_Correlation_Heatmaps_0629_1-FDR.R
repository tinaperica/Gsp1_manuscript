...
# load FDR corrected values
load('Data/filtered_correlations.RData')
...
corr_mat <-
  ...
  filter(query_uniq2 %in% queries_with_two_sig_corr) %>% 
...
# save heatmap
pdf('Revisions2/4A_Corr_Pvalue_Heatmap_0629_FDR_filter.pdf', width = 4.5, height = 6.7)
draw(pval_heatmap)
