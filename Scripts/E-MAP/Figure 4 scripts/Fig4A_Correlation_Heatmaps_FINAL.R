...
## Get the counts in each cluster of alleles
order_of_alleles <- labels(row.hc)

df_cluster <-
  cutree(row.hc, k = 7) %>% 
  stack() %>% 
  as_tibble() %>% 
  rename('cluster_cutting_order'='values', 'allele'='ind') %>%
  mutate(allele=factor(allele, levels=order_of_alleles)) %>% 
  arrange(allele)

cluster_mapping <-
  df_cluster %>% 
  select(cluster_cutting_order) %>% 
  unique() %>% 
  bind_cols(data.frame(cluster_figure_order=c(1,2,3,4,5,6,7)))

print('Number of alleles in each cluster')  
df_cluster %>% 
  left_join(cluster_mapping, by='cluster_cutting_order') %>% 
  select('cluster'=cluster_figure_order, allele) %>% 
  group_by(cluster) %>% 
  summarise(n())
...
pval_heatmap <- Heatmap(
  ...
  row_dend_gp = gpar(lwd=0.5), column_dend_gp = gpar(lwd=0.5),

  # columns (mutants)
  column_title = 'Gsp1 point mutant', column_title_gp = gpar(fontsize = 7),
  ...
  bottom_annotation = HeatmapAnnotation(
    ...
    height = unit(5.4, "cm"),
    gap = unit(c(4, 4), "mm"),
    annotation_height = unit(c(1.4, 1.4, 1.4), "cm"),
  ...
  # rows (genes)
  row_title = 'Saccharomyces cerevisiae alleles', row_title_gp = gpar(fontsize = 7),
...
add_error_bars_and_stars <- function(annot, val, err) {
  decorate_annotation(annot, {
    ...
    x =  c(seq(1,6), seq(7.372, 13.372), seq(14.744,22.744)) 
    x0 = c(seq(1,6), seq(7.372, 13.372), seq(14.744,22.744)) - err_w
    x1 = c(seq(1,6), seq(7.372, 13.372), seq(14.744,22.744)) + err_w
    ...
    if (annot == 'in vitro\nGAP relative\nefficiency\n(MUT/WT)') {
      x_star = c(18.744, 19.744, 20.744)
      y_star = rep(0.4, 3)
    } else if (annot == 'in vitro\nGEF relative\nefficiency\n(MUT/WT)') {
      x_star = c(13.372, 18.744, 19.744, 20.744)
      y_star = rep(0.1, 4)
...
# save heatmap
pdf('Final_Formatting/temp/Fig4A.pdf', width = 3.4, height = 5.8)
draw(pval_heatmap)
