...
GAP_kcat <- get_ordered_kinetics_df('GAP_kcat')
GAP_Km <- get_ordered_kinetics_df('GAP_Km')
GEF_kcat_Km <- get_ordered_kinetics_df('GEF_kcat_Km')
GEF_kcat <- get_ordered_kinetics_df('GEF_kcat')
GEF_Km <- get_ordered_kinetics_df('GEF_Km') %>% 
  mutate('rel_to_WT' = ifelse(rel_to_WT > 30, 30, rel_to_WT))
...
pval_heatmap <- Heatmap(
  ...
  width = unit(6.2, 'cm'), gap = unit(1, "mm"),
  ...
  bottom_annotation = HeatmapAnnotation(
    which = 'col',
    height = unit(5.5, "cm"), 
    gap = unit(c(3.5, 3.5, 3.5, 3.5, 3.5, 3.5), "mm"),
    annotation_height = unit(c(1.75, 1.75, 1.75, 1.75, 1.75, 1.75, 2), "cm"),
    
    `in vitro\nGAP relative\nkcat\n(MUT/WT)` = anno_barplot(
      x = GAP_kcat$rel_to_WT,
      ylim = c(0, 1.5),
  ...
  cluster_rows = as.dendrogram(row.hc), row_split = 7, row_dend_width = unit(8, "mm"),
...
#     x = c(seq(1,6), seq(7.37, 13.37), seq(14.73,22.73))
#     x0 = c(seq(1,6), seq(7.37, 13.37), seq(14.73,22.73)) - err_w
#     x1 = c(seq(1,6), seq(7.37, 13.37), seq(14.73,22.73)) + err_w
#     
#     # y positions
#     y = val[co]
#     y0 = val[co] - err[co]
...
#     if (annot == 'in vitro\nGAP relative\nefficiency\n(MUT/WT)') {
#       x_star = c(18.73, 19.73, 20.73)
#       y_star = rep(0.4, 3)
#     } else if (annot == 'in vitro\nGEF relative\nefficiency\n(MUT/WT)') {
#       x_star = c(13.37, 18.73, 19.73, 20.73)
#       y_star = rep(0.1, 4)
...
# save heatmap
pdf('Revisions/Main Figures/Figure4/Heatmap_kcat_Km.pdf', width = 4.5, height = 10)
draw(pval_heatmap)
...
dev.off()
# 
pdf('Revisions/Main Figures/Figure4/4A_Kinetics_Legend.pdf', width = 1, height = 1)
barchart_legend <- Legend(title = 'log(GAP eff - GEF eff)',
       col_fun = rc_col_fn,
       at = c(-2, 0, 3.5),
...
# plotting heatmap - basically all the same as pval_heatmap except it takes pcc_cor_mat and the colorRamp2 is different
#### maybe rewrite as function?
pcc_heatmap <- Heatmap(
  pcc_corr_mat, show_heatmap_legend = F,
  ...
  bottom_annotation = HeatmapAnnotation(
    which = 'col',
    height = unit(5.5, "cm"), 
    gap = unit(c(3.5, 3.5), "mm"),
    annotation_height = unit(c(1.75, 1.75, 2), "cm"),
...
co <- unlist(column_order(pcc_heatmap))
dev.off()
...
# save
pdf('Revisions/Extended_Figures/EDF_9/Ext_Fig9A_PCC_Heatmap.pdf', width = 4.5, height = 6.7)
draw(pcc_heatmap)
add_error_bars_and_stars('in vitro\nGAP relative\nefficiency\n(MUT/WT)',
                         GAP_kcat_Km$rel_to_WT, GAP_kcat_Km$se)
...
pdf('Revisions/Extended_Figures/EDF_9/Ext_Fig9A_PCC_Legend.pdf', width = 1, height = 1)
draw(pcc_heatmap_legend)
dev.off()

pdf('Revisions/Extended_Figures/EDF_9/Ext_Fig9A_Kinetics_Legend.pdf', width = 1, height = 1)
draw(barchart_legend)
dev.off()
