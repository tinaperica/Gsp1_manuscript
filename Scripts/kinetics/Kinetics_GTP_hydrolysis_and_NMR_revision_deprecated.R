...
filtered_temp <- temp %>% 
  filter(! mutant %in% c('R78K', 'D79S', 'K132H'))
GAPNMRfit <- lm(GAP ~ NMR, data = filtered_temp)
ln_rel_GAP_gamma2_plot <- temp %>% 
  ggplot(aes(y = GAP, x = NMR)) + 
  geom_point(size = 2, alpha = 0.5) +
  geom_errorbar(aes(ymin = GAP - GAP_se, ymax = GAP + GAP_se), width = 2, size = 0.1) + 
...
ggsave(filename = str_c(main_directory, 'Fig3d_GAP_kcatKm_vs_NMR.pdf'), height = 2.05, width = 2.22)


### make a plotting function that does just kcat or just Km
plot_MM_parameter_gamma <- function(measure, input_data) {
  plot<- input_data %>% 
  ggplot(aes(y = value, x = NMR)) + 
  geom_point(size = 2, alpha = 0.5) +
  geom_errorbar(aes(ymin = value - se, ymax = value + se), width = 2, size = 0.1) + 
  geom_text_repel(aes(label = mutant), size = 2, point.padding = 0.1, segment.size = 0.3) +
  ylab(measure) +
  xlab(expression("\n% "*gamma*" phosphate in "*gamma*" state 2")) +
  theme_classic() +
...
kcat_temp <- rel_data %>% 
  ...
  select(mutant, "value" = rel_to_WT, se)
temp <- rel_data %>% 
  filter(measure == 'NMR') %>% 
  select(mutant, 'NMR' = rel_to_WT) %>% 
...
ggsave(filename = str_c(extended_directory, 'GAP_Km_vs_NMR_with_zoom.pdf'), height = 2.3, width = 3)
...
intNMRfit <- lm(int ~ NMR, data = temp)
temp %>% 
  ggplot(aes(y = int, x = NMR)) + 
  geom_point(size = 2, alpha = 0.5) +
  geom_errorbar(aes(ymin = int - ln_se, ymax = int + ln_se), width = 2, size = 0.1) + 
  ...
  theme(text = element_text(size = 6),
        ...
        axis.line = element_line(size = 0.1),
        axis.ticks = element_line(size = 0.1))
ggsave(file.path(extended_directory, 'EDFig8_intrinsic_vs_NMR_state_2_scatterplot.pdf'), height = 2.5, width = 2.5)
