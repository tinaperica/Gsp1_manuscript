...
main_directory <- 'Final_Formatting/kinetics'
extended_directory <- 'Final_Formatting/kinetics/extended'

std.error.product <- function(x, se.x, y, se.y) {
  sqrt( (se.x/x)^2 + (se.y/y)^2 ) * x/y
}
...
GAP.data_unaveraged <- GAP.data %>% # this is to add individual point to barplots
  select(mutant, kcat, Km, 'kcat_Km' = individual_kcat_Km)
GAP.data <- GAP.data %>% 
  select(mutant, 'kcat' = mean_kcat, 'Km' = mean_Km, 
        'kcat_Km' = mean_kcat_Km,	kcat_se, Km_se, se) %>% 
  unique()
GEF.data <- GEF.data %>% 
  filter(mutant %in% intersect_mutants)
...
gap_plot <- GAP.data %>% 
  ...
  geom_errorbar(aes(ymin = kcat_Km - se, ymax = kcat_Km + se), width = 0.5, size = 0.2, alpha = 0.75) +
  geom_point(data = GAP.data_unaveraged, aes(x = mutant, y = kcat_Km), size = 0.4, fill = 'black', alpha = 0.4) +
...
gap_kcat_plot <- GAP.data %>% 
  ...
  geom_bar(stat = "identity") + 
  geom_errorbar(aes(ymin = kcat - kcat_se, ymax = kcat + kcat_se), width = 0.5, size = 0.2, alpha = 0.75) +
  geom_point(data = GAP.data_unaveraged, aes(x = mutant, y = kcat), size = 0.4, alpha = 0.4, fill = 'black') +
...
gap_Km_plot <- GAP.data %>% 
  ...
  geom_bar(stat = "identity") + 
  geom_errorbar(aes(ymin = Km - Km_se, ymax = Km + Km_se), width = 0.5, size = 0.2, alpha = 0.75) +
  geom_point(data = GAP.data_unaveraged, aes(x = mutant, y = Km), size = 0.4, fill = 'black', alpha = 0.4) +
...
gef_plot <- GEF.data %>% 
  ...
  geom_errorbar(aes(ymin = kcat_Km - kcat_Km_se, ymax = kcat_Km + kcat_Km_se), width = 0.5, size = 0.2, alpha = 0.75) +
  ylab(expression("k"['cat']*" / K"['m']*" [s "^-{1}*mu*"M"^-{1}*"]")) +
  xlab('\nGsp1 point mutant') +
...
ggsave(file.path(main_directory, '3AB_gap_and_gef_kcat_Km_barplots.pdf'), width = 2.15, height = 6)
...
# Plot Extended Data Figure 6abcd - Separate kcat and Km, GAP and GEF
plot_grid(gap_kcat_plot, gap_Km_plot, gef_kcat_plot, gef_Km_plot, align = c('v', 'h'), nrow = 2, rel_widths=c(1,1.5)) +
  draw_plot(gef_Km_plot_zoom, x = 0.55, y = 0.1, width = 0.39, height = 0.4)
ggsave(file.path(extended_directory, 'Ext_Fig6ABCD_gap_and_gef_kcat_and_Km_supp_barplots.pdf'), width = 6, height = 5)



# Extended Data Figure 6e, relative GAP and GEF data on the same barplot
WT_GAP <- GAP.data %>% 
  filter(mutant == "WT") %>% 
  select(mutant, 'WT_kcat_Km' = kcat_Km, 'WT_se' = se)
rel_GAP.data <- GAP.data %>% 
  mutate('WT_kcat_Km' = WT_GAP$WT_kcat_Km, 'WT_se' = WT_GAP$WT_se) %>% 
  mutate('rel_GAP_kcat_Km' = kcat_Km/WT_kcat_Km, 
         'rel_GAP_se' = std.error.product(x = kcat_Km, se.x = se, y = WT_kcat_Km, se.y = WT_se)) %>% 
  select(mutant, rel_GAP_kcat_Km, rel_GAP_se)
WT_GEF <- GEF.data %>% 
  filter(mutant == "WT") %>% 
  select(mutant, 'WT_kcat_Km' = kcat_Km, 'WT_se' = kcat_Km_se)
rel_GEF.data <- GEF.data %>% 
  mutate('WT_kcat_Km' = WT_GEF$WT_kcat_Km, 'WT_se' = WT_GEF$WT_se) %>% 
  mutate('rel_GEF_kcat_Km' = kcat_Km/WT_kcat_Km, 
         'rel_GEF_se' = std.error.product(x = kcat_Km, se.x = kcat_Km_se, y = WT_kcat_Km, se.y = WT_se)) %>% 
  select(mutant, rel_GEF_kcat_Km, rel_GEF_se)
...
data <- inner_join(rel_GAP.data, rel_GEF.data, by = 'mutant') %>% 
  ...
  gather(key = 'measure', value = 'value', -mutant, -rel_GAP_se, -rel_GEF_se, -GAP_GEF_ratio) %>% 
  mutate('se' = ifelse(measure == 'rel_GAP_kcat_Km', rel_GAP_se, rel_GEF_se)) %>% 
...
data %>% 
  ...
  geom_errorbar(aes(ymin = value - se, ymax = value + se), width = 0.3, size = 0.2, alpha = 0.75, position = position_dodge(0.9)) +
  theme_classic() +
  labs(fill = element_blank()) +
...
ggsave(filename = file.path(extended_directory, "Ext_Fig6_e_GAP_GEF_combined_barplot_kcat_over_Km.pdf"), height = 3, width = 4.5)
...
intrinsic_hydrolysis <- intrinsic_hydrolysis %>% 
  select(mutant, mean_rel_rate, se_rel_rate) %>% unique()
...
intrinsic_hydrolysis %>% 
  ...
  geom_bar(stat = "identity") + 
  geom_errorbar(aes(ymin = mean_rel_rate - se_rel_rate, ymax = mean_rel_rate + se_rel_rate), width = 0.5, size = 0.2, alpha = 0.75) +
  geom_point(data = intrinsic_hydrolysis_unaveraged, aes(x = mutant, y = rel_rate), fill = 'black', size = 0.4, alpha = 0.4) +
