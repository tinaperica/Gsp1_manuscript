...
directory <- 'Biophysics_Figure3/GTP_hydrolysis_and_NMR_plots'

# NMR data
ordering <- c('H141R','H141V','K132H','Y157A','D79S','WT',
...
outliers <- GAP_NMR_spread %>% 
  ...
  summarise('ymin' = min(kcat_Km) - abs(0.1 * min(kcat_Km)), 'ymax' = max(kcat_Km) + abs(0.1 * max(kcat_Km)), 'xmin' = min(NMR) - abs(0.1 * min(NMR)), 'xmax' = max(NMR) + abs(0.1 * max(NMR)))
...
ggsave(file.path(directory, 'GAP_vs_NMR_state_2_scatterplot.pdf'), height = 1.9, width = 2)
...
int_NMR_spread %>% 
  ...
  theme(text = element_text(size = 6),
        ...
        axis.title.y = element_text(size = 6),
        axis.line = element_line(size = 0.1))
ggsave(file.path(directory, 'intrinsic_vs_NMR_state_2_scatterplot.pdf'), height = 1.9, width = 2)
...
int_GAP_spread %>% 
  ...
  theme(text = element_text(size = 6),
        ...
        axis.title.y = element_text(size = 6),
        axis.line = element_line(size = 0.1))
ggsave(file.path(directory, 'intrinsic_vs_GAP_scatterplot.pdf'), height = 3, width = 3)
